    ...
    <div className="p-6">
      <div className="bg-white squircle shadow-sm p-8 flex flex-col items-center text-center mb-6 border border-gray-50">
        ...
        <div className="relative mb-6">
          <div className="w-32 h-32 rounded-full p-1 bg-gradient-to-tr from-[#FA3D67] to-[#FF8FA7] shadow-lg">
            <div className="w-full h-full rounded-full border-4 border-white overflow-hidden bg-brand-light flex items-center justify-center">
              {profile.photo ? (
                <img 
                  src={profile.photo} 
                  alt="" 
                  ...
                  onError={(e) => {
                    // Если картинка не загрузится, скроем её и покажем инициалы
                    (e.target as HTMLImageElement).style.display = 'none';
                  }}
                />
              ) : null}
              <span className="text-brand font-black text-2xl absolute pointer-events-none">
                {profile.name.split(' ').map(n => n[0]).join('')}
              </span>
            </div>
          </div>
          <div className="absolute bottom-1 right-1 bg-[#FA3D67] border-[3px] border-white w-9 h-9 rounded-full flex items-center justify-center shadow-md">
